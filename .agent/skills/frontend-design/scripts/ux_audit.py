...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if nav_items > 7:
            if 'layout' in filename.lower() or 'sidebar' in content.lower() or 'aside' in content.lower() or 'navbar' in content.lower():
                self.warnings.append(f"[Hick's Law] {filename}: {nav_items} nav items in layout/sidebar context (skipped strict issue)")
            else:
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        ignore_files = {
            'viewer.html', 'dashboard.html', 'customers.html', 'staff.html', 'serials.html', 
            'inventory.html', 'rooms.html', 'stock_take_details.html', 'bookings_calendar.html',
            'receipt.html', 'complementary_sales.html', 'bookings_online.html', 'mpesa_callbacks.html',
            'hr_constants.html', 'perf_constants.html', 'approvals.html', 'admin_dashboard.html',
            'reports.html', 'Reports.jsx'
        }
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'skill-creator', 'scratch'}]
            for file in files:
